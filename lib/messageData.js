...
export default class MessageModel {
    ...
    async getCountimages() {
        try {
            const results = await this.client.query(`SELECT COUNT(*) FROM messages WHERE type = 'image'`);
            return results.rows[0]['count'];
        } catch (error) {
            console.error('Failed to get messages', error);
        }
    ...
    async getCountvideos() {
        try {
            const results = await this.client.query(`SELECT COUNT(*) FROM messages WHERE type = 'video'`);
            return results.rows[0]['count'];
        } catch (error) {
            console.error('Failed to get messages', error);
        }
